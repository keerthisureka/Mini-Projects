...
def book(a, b, c, d):
    ...
    apptid = [item[0] for item in cursor]
    apptid = int(apptid[0]) + 1
    cursor.execute(f"SELECT UserID FROM user WHERE Email='{a}'")
    userid = [item[0] for item in cursor][0]
    cursor.execute(f"SELECT LabID FROM lab WHERE LabName='{b}'")
    labid = [item[0] for item in cursor]
    cursor.execute(f"SELECT TestID FROM test WHERE TestName='{c}'")
    testid = [item[0] for item in cursor]
    cursor.execute(f"""INSERT INTO appointment VALUES ({apptid}, '{userid}', '{labid}', '{testid}', '{d}')""")
    cursor.execute(f"SELECT t.TestName, l.LabName, l.Location, e.Price, a.ApptDate FROM test t, lab l, efficiency e, user u, appointment a WHERE u.UserID='{userid}")
    details_view = cursor.fetchall()
    return details_view
